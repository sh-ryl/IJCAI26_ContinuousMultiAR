...
class NeuralQLearner(agent.Agent):
    ...
    def __init__(self, name, agent_params, transition_params):
        ...
        self.optimizer = optim.Adam(self.network.parameters(), lr=agent_params["adam_lr"], betas=(
            agent_params["adam_beta1"], agent_params["adam_beta2"]), eps=agent_params["adam_eps"])
    ...
    def add_episode_to_cache(self):
        ...
        for i in range(0, ep_length):
            self.transitions.add(self.current_episode[i][IDX_STATE], self.current_episode[i][IDX_ACTION], self.current_episode[i]
                                 [IDX_EXTRINSIC_REWARD], ret[i], ret_partial[i], self.current_episode[i][IDX_TERMINAL], ep_length - 1 - i)
            i = i + 1
    ...
    def handle_terminal(self):
        ...
        self.moving_average_score = self.moving_average_score_mom * \
            self.moving_average_score + \
            (1.0 - self.moving_average_score_mom) * self.episode_score
        self.moving_average_score_clipped = self.moving_average_score_mom * \
            self.moving_average_score_clipped + \
            (1.0 - self.moving_average_score_mom) * self.episode_score_clipped

        self.moving_average_score_updates = self.moving_average_score_updates + 1

        zero_debiased_score = self.moving_average_score / \
            (1.0 - self.moving_average_score_mom **
             self.moving_average_score_updates)
        zero_debiased_score_clipped = self.moving_average_score_clipped / \
            (1.0 - self.moving_average_score_mom **
             self.moving_average_score_updates)
    ...
    def perceive(self, reward: float, state: CraftWorldState, terminal: bool, is_eval: bool):
            ...
            if self.lastState is not None:
                ...
                if self.post_episode_return_calcs_needed:
                    ...
                    self.transitions.add(
                        self.lastState, self.lastAction, reward, 0.0, self.lastTerminal)
        ...
        if not terminal:
            if self.exploration_style == "e_greedy":
                actionIndex = self.eGreedy(
                    state.to(self.device).unsqueeze(0), is_eval)
            elif self.exploration_style == "e_softmax":
                actionIndex = self.eSoftmax(
                    state.to(self.device).unsqueeze(0), is_eval)
            else:
                print("ERROR: Unrecognised exploration_style (" +
                      self.exploration_style + ")")
                sys.exit(0)

        if not is_eval:
            self.q_values_plot.add_data_point(
                "bestq", self.numSteps, self.bestq, True, self.show_graphs)
    ...
    def learn(self):
        ...
        ) > self.minibatch_size, 'Not enough transitions stored to learn'

        s, a, _, ret, ret_partial, s2, _, term_under_n = self.transitions.sample(
            self.minibatch_size)
        ...
        target_overall = torch.ones_like(term_under_n).sub(term_under_n).mul(
            self.discount ** self.n_step_n).mul(value_tp1).add(ret_partial)

        mixed_monte_carlo_proportion = self.mixed_monte_carlo_proportion_start * \
            max(0, 1 - (self.numSteps / self.mixed_monte_carlo_proportion_endt))
        target_overall = target_overall.mul(
            1 - mixed_monte_carlo_proportion).add(ret.mul(mixed_monte_carlo_proportion))
    ...
    def load_model(self, path):
        ...
        self.optimizer = optim.Adam(self.network.parameters(), lr=self.agent_params["adam_lr"], betas=(
            self.agent_params["adam_beta1"], self.agent_params["adam_beta2"]), eps=self.agent_params["adam_eps"])
    ...
    def eSoftmax(self, state, is_eval):
        if is_eval or self.test_mode:
            ...
            self.ep = self.ep_end + np.maximum(0, (self.ep_start - self.ep_end) * (
                self.ep_endt - np.maximum(0, self.numSteps - self.learn_start)) / self.ep_endt)

        a = self.softmax(state)

        # Epsilon softmax
        if np.random.uniform() < self.ep:
            a = randrange(self.n_actions)
            return a
        else:
            return a
